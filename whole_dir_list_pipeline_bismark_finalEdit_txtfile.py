    ...
    with open(get_uniq_outfile, "w") as outfile:
        for each_file in file_list:
            with open(each_file, "r") as file:
                ...
                #For Total_raw_reads from Trim galore file:
                #grep "Total number of sequences analysed:" log_files.out

                # In [12]: with open("/gpfs/gpfs1/home/schhetri/SL83770/trim_galore_raw_read.txt", "r") as file:
                #     data_file = file.read()
                #     pattern1="Total number of sequences analysed:\s(\d+)"
                #     regex_next = re.compile(pattern1)
                #     read_count = regex_next.findall(data_file)
                #    ....:
                
                # In [13]: paired_read_count_int = map(int, read_count)

                # In [14]: Total_read_count = sum(paired_read_count_int)*2
